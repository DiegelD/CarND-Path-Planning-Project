      ...
      if (s != "")
      {
        ...
        if (event == "telemetry")
        {
          ...
          { // Car is on the very left lane
            cout << "4" << endl;
            if (car.current_state.compare("PLCR") == 0  )
            {
              cout << "4.1.0" << endl;
            ...
            else if (car.current_state.compare("PLCL") == 0)
            {
          ...
          {
            double N = (target_dist / (.02 * ref_vel / 2.24)); // .02 is the update time  2.24 from mph need to be in m/s
