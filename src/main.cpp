...
int main()
{
  ...
  h.onMessage([&map_waypoints_x, &map_waypoints_y, &map_waypoints_s,
               &map_waypoints_dx, &map_waypoints_dy, &car, &ref_vel](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                                                                                         uWS::OpCode opCode) {
      ...
      if (s != "")
      {
        ...
        if (event == "telemetry")
        {
          ...
          if (prev_size > 0)
          {
            ...
            vector<Car> predictions = car.generate_predictions(sensor_fusion, car, prev_size);
            vector<Car> trajectory = car.choose_next_state( predictions);
            ...
            {
              double N = (target_dist / (.02 * car.v / 2.24)); // .02 is the update time  2.24 from mph need to be in m/s
